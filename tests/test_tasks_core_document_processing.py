...
def test_document_saving_task_success(tmp_path, monkeypatch):
    ...
    class FakeOutputManager:
        ...
        def save_processed_document(
            self, document_id, processed_content, metadata, user_id="default"
        ):
    ...
    monkeypatch.setattr(
        document_saving_task, "DocumentOutputManager", FakeOutputManager
    )
    ...
    monkeypatch.setattr(
        document_saving_task,
        "create_message",
        lambda event_type, data, source: json.dumps(
            {"event_type": event_type, "data": data, "source": source}
        ),
    )

    res = document_saving_task.document_saving_task.run(
        str(md),
        "docsave",
        content_length=123,
        page_count=1,
...
def test_duplicate_detection_task_duplicate_and_ready(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        duplicate_detection_task, "DocumentCRUD", lambda cm: FakeCrudDup()
    )
    ...
    monkeypatch.setattr(
        duplicate_detection_task, "DocumentCRUD", lambda cm: FakeCrudNew()
    )
