...
async def document_processing_health(request: Request):
    ...
    try:
        ...
        services["file_system"] = (
            "accessible" if os.access(upload_dir, os.W_OK) else "inaccessible"
        )
    except Exception:
    ...
    all_healthy = all(
        status in ("connected", "accessible") for status in services.values()
    )
    status = "healthy" if all_healthy else "degraded"
    status_code = 200 if all_healthy else 503

    return HealthResponse(
        status=status, timestamp=datetime.utcnow(), services=services, version="1.0.0"
    )
