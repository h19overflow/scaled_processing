    ...
    try:
        ...
        if isinstance(input_data, bytes) and "Data format error" in str(e):
            ...
            try:
                ...
                return _original_pdf_document_init(
                    self, repaired_bytes, password, autoclose
                )
    ...
    if backend == "pipeline":
        for idx, pdf_bytes in enumerate(pdf_bytes_list):
            try:
                ...
                logger.warning(
                    f"pypdfium2 failed for PDF {idx}: {e}. Attempting repair..."
                )
                try:
                    repaired_bytes = repair_pdf_fallback(pdf_bytes)
                    new_pdf_bytes = convert_pdf_bytes_to_bytes_by_pypdfium2(
                        repaired_bytes, start_page_id, end_page_id
                    )
                    pdf_bytes_list[idx] = new_pdf_bytes
        ...
        for idx, model_list in enumerate(infer_results):
            ...
            local_image_dir, local_md_dir = custom_prepare_env(
                output_dir, pdf_file_name
            )
            ...
            middle_json = pipeline_result_to_middle_json(
                model_list, images_list, pdf_doc, image_writer, _lang, _ocr_enable
            )
        ...
        for idx, pdf_bytes in enumerate(pdf_bytes_list):
            ...
            try:
                ...
                logger.warning(
                    f"pypdfium2 failed for PDF {pdf_file_name}: {e}. Attempting repair..."
                )
                try:
                    repaired_bytes = repair_pdf_fallback(pdf_bytes)
                    pdf_bytes = convert_pdf_bytes_to_bytes_by_pypdfium2(
                    ...
                    logger.info(
                        f"PDF {pdf_file_name} successfully repaired and processed"
                    )
                except Exception as repair_error:
                    logger.error(
                        f"PDF repair failed for {pdf_file_name}: {repair_error}"
                    )
                    raise e  # Re-raise original error if repair fails
...
if __name__ == "__main__":
    ...
    for pdf_file in individual_files:
        if pdf_file.exists():
            try:
                parse_single_file(
                    file_path=pdf_file, output_dir=output_dir, backend="pipeline"
                )
            except Exception as e:
