...
class DocumentOutputManager:
    ...
    def __init__(self, processed_documents_dir: str = "data/documents/processed"):
        ...
        self.logger.info(
            f"DocumentOutputManager initialized with output dir: {self.processed_dir}"
        )

    def check_and_process_document(
        self, raw_file_path: str, user_id: str = "default"
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            is_duplicate, existing_doc_id = (
                self.document_crud.check_duplicate_by_raw_file(str(raw_path))
            )

            if is_duplicate:
                self.logger.info(
                    f"Document is duplicate, skipping processing: {existing_doc_id}"
                )
                return {
                    "status": "duplicate",
            ...
            try:
                ...
                self.logger.info(
                    f"Document record created in database: {db_document_id}"
                )
        ...
        try:
            ...
            path_info = {
                ...
                "exists": {
                    ...
                    "metadata": (
                        document_dir / f"{document_id}_metadata.json"
                    ).exists(),
                },
            }
            ...
            self.logger.error(
                f"Error getting document path info for {document_id}: {e}"
            )
            return {
                "status": "error",
                "error": str(e),
    ...
    def _generate_document_id(self, file_path: Path) -> str:
        ...
        safe_id = safe_id.strip(
            ". _"
        )  # Strip leading/trailing spaces, periods, and underscores
    ...
    def _store_document_in_database(
        self, document_id: str, metadata: Dict[str, Any], user_id: str
    ):
        ...
        try:
            ...
            if raw_file_path:
                ...
                content_hash = self.document_crud.generate_content_hash_from_bytes(
            ...
            if existing_doc:
                self.logger.info(
                    f"Document with same content hash already exists in database: {existing_doc.get_id()}"
                )
                return
            ...
            self.logger.info(
                f"Document stored in database with generated ID: {db_document_id}"
            )
