...
class TableLineItemExtractor:
    ...
    def extract_tables_from_content_list(self, content_list_path: Path, output_csv_path: Path, document_id: str) -> bool:
        ...
        try:
            ...
            table_elements = [item for item in content_list if item.get('type') == 'table']
    ...
    def _convert_dataframe_to_line_items(self, df: pd.DataFrame, document_id: str, start_id: int, table_index: int) -> List[Dict[str, Any]]:
        ...
        for row_index, row in df.iterrows():
            ...
            line_item_label = str(first_col_value).strip() if first_col_value is not None else f"row_{row_index}"
            ...
            for col_index in range(len(row)):
                ...
                col_name = column_names[col_index] if col_index < len(column_names) else f"column_{col_index}"
            ...
            if header_row is not None:
                ...
                for col_idx in range(len(df.columns)):
                    if col_idx < len(header_row) and pd.notna(header_row.iloc[col_idx]):
                        ...
                        header_names.append(col_name if col_name else f"column_{col_idx}")
                    else:
    ...
    def _find_header_row(self, df: pd.DataFrame) -> pd.Series:
        ...
        for idx, row in df.iterrows():
            ...
            if len(row_text) > 5 and not row_text.replace(' ', '').replace('.', '').replace(',', '').isdigit():
