...
def document_processing_task(
    ...
    try:
        ...
        logger.info(
            f"🔥 AFTER processor.extract_document() call - Status: {extraction_result.get('status')}"
        )

        if extraction_result["status"] == "completed":
            logger.info(f"✅ Docling processing completed for: {document_id}")
            logger.info(
                f"📝 Markdown saved to: {extraction_result['processed_markdown_path']}"
            )
            ...
            logger.error(
                f"❌ Docling processing failed for {document_id}: {extraction_result.get('error')}"
            )
            return {
                "status": "error",
                "document_id": document_id,
