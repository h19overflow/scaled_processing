...
def document_saving_task(
    ...
    entry_debug_file = os.path.join(
        debug_dir, f"ENTRY_{document_id}_function_called.txt"
    )
    ...
    try:
        ...
        if not enhanced_markdown_path.exists():
            raise FileNotFoundError(
                f"Vision-enhanced markdown not found: {markdown_path}"
            )
        ...
        logger.info(
            f"📖 Read {len(processed_content)} characters from vision-enhanced markdown"
        )
        ...
        if save_result.get("status") == "saved" and save_result.get(
            "processed_file_path"
        ):
            try:
                with open(
                    save_result["processed_file_path"], "r", encoding="utf-8"
                ) as f:
                    final_processed_content = f.read()
                logger.info(
                    f"📖 Read {len(final_processed_content)} characters from saved file: {save_result['processed_file_path']}"
                )
        ...
        debug_file = os.path.join(
            debug_dir, f"{document_id}_final_processed_content.txt"
        )
        try:
            with open(debug_file, "w", encoding="utf-8") as f:
                ...
                f.write(
                    f"Saved file path: {save_result.get('processed_file_path', 'N/A')}\n"
                )
        ...
        try:
            ...
            result = kafka_producer.produce_message(
                topic="document_pipeline_completed",
            ...
            if result:
                ...
                logger.error(
                    f"❌ Failed to send message for document: {file_path_obj.name}"
                )

        except Exception as e:
            logger.error(
                f"Failed to send message for document: {file_path_obj.name} ERROR: {e}"
            )

        if save_result["status"] == "saved":
            logger.info(
                f"✅ Document saved successfully: {save_result['processed_file_path']}"
            )
        else:
...
def main():
    ...
    try:
        ...
        if result.get("kafka_message"):
            ...
            print(
                f"Processed content length: {len(message_obj.get('data', {}).get('processed_content', ''))}"
            )
