...
def store_in_database(state: PipelineState) -> dict[str, Any] | None:
    ...
    try:
        ...
        logger.info(
            f"📋 Storage - document_id: '{document_id}', document_name: '{document_name}'"
        )
        ...
        if existing_bill_id:
            logger.info(
                f"Duplicate detected: Bill already exists for document_id '{document_id}' (document: '{document_name}') with ID: {existing_bill_id}"
            )
            return {
                "status": "storage_skipped",
        ...
        try:
            bill_id = _create_and_store_bill(
                extractions, document_name, document_id, connection_manager
            )
            return {
            ...
            logger.info(
                f"Duplicate bill detected (race condition): document '{document_name}' already exists"
            )
            return {
                "status": "storage_skipped",
            ...
            return {"status": "storage_error", "error": str(e), "stored_count": 0}
...
def _create_and_store_bill(
    ...
    for extraction in extractions:
        ...
        logger.info(
            f"Processing extraction: class={extraction_class}, text='{extraction_text}', attributes={attributes}"
        )
        ...
        if extraction_class in CORE_FIELDS:
            ...
            if extraction_class == "amount_due":
                ...
                if parsed_amount is not None:
                    ...
                    logger.warning(
                        f"⚠️ Failed to parse amount_due from attributes: {attributes}"
                    )
                ...
                if parsed_date is not None:
                    ...
                    logger.warning(
                        f"⚠️ Failed to parse due_date from text: '{extraction_text}', attributes: {attributes}"
                    )
                ...
                if parsed_date is not None:
                    ...
                    logger.warning(
                        f"⚠️ Failed to parse issue_date from text: '{extraction_text}', attributes: {attributes}"
                    )
        else:
    ...
    logger.info(
        f"💾 Creating BillModel - amount_due: {amount_due_decimal}, issue_date: {core_data['issue_date']}, due_date: {core_data['due_date']}"
    )

    # Create BillModel instance

    # Store in database
    bill_id = BillCRUD(connection_manager).create(
...
def _parse_date(date_text: str, attributes: dict = None) -> Optional[datetime]:
    ...
    if attributes and "iso_date" in attributes:
        try:
            ...
            logger.warning(
                f"Failed to parse ISO date '{attributes.get('iso_date')}': {e}"
            )
...
def _parse_amount(attributes: dict = None) -> Optional[float]:
    ...
    if "amount_due" in attributes:
        try:
            ...
            logger.error(
                f"Failed to parse amount '{attributes.get('amount_due')}': {e}"
            )
