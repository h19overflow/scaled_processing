...
class ExtractionResult(BaseModel):
    ...
    extraction_class: str = Field(
        description="Type of extraction (postal_address, invoice_number, etc.)"
    )
    extraction_text: str = Field(description="Original text that was extracted")
    attributes: Dict[str, Any] = Field(
        description="Structured attributes for the extraction"
    )
...
class BillExtractionResult(BaseModel):
    ...
    previous_balance: Optional[float] = Field(
        None, description="Previous balance amount"
    )
